...
### 1.4 Relación con el Portal de Empleo API

Este caso práctico es una extensión natural del proyecto Portal de Empleo API desarrollado previamente. TalentForecast Pro consume datos del Portal y los transforma en inteligencia accionable, estableciendo una relación de dependencia directa.

| Componente Portal API | Componente TalentForecast Pro |
|----------------------|-------------------------------|
| User, JobOffer, Application Entities | Consumidos via httpx desde Portal API |
| Repository Pattern | Base para acceso a datos |
| AuthController (JWT) | Reutilizado para autenticación de analytics API |
| Clean Architecture | Estructura de referencia para src/ |
| AGENTS.md | Extendido con agentes especializados data |

### 1.5 Relación con el Curso

Este caso práctico está diseñado para aplicarse en paralelo con los módulos del curso, aplicando SDD híbrido específicamente adaptado para proyectos de machine learning:

| Módulo del Curso | Enfoque SDD + ML | Duración | Práctica del Caso |
|-------------------|------------------|----------|-------------------|
| **Módulo 1** | Constitución ML + Prompts Data | 3h | Configuración AGENTS.md data + Biblioteca de prompts ML |
| **Módulo 2** | Spec de Hipótesis + Plan Experimental | 3h | specs/001-*/spec.md con hipótesis + plans con gates |
| **Módulo 3** | Tasks con Gates + Implementación ML | 3h | Feature engineering + Model training + API |
| **Módulo 4** | MLOps + Validación + Docs | 3h | MLflow tracking + Tests + CI/CD |

### 1.6 Recursos Disponibles

Para el desarrollo de este proyecto, el equipo cuenta con acceso al Portal de Empleo API ya desarrollado, licencias de Microsoft Copilot para todos los desarrolladores, formación sobre Spec-Driven Development híbrido con checkpoints experimentales, repositorio Git configurado con Git LFS para datasets grandes, entorno de desarrollo con Python 3.11+, Jupyter notebooks para experimentación, y acceso a compute resources para entrenamiento de modelos.
