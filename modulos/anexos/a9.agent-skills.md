...
| **Project skills** | `.agents/skills/` | Alternativa moderna |
...
| **Personal skills** | `~/.agents/skills/` | Alternativa moderna |
| **Personal skills (legacy)** | `~/.claude/skills/` | Para compatibilidad hacia atrás |

### Configuración adicional

Puedes configurar ubicaciones adicionales para project skills con el setting `chat.skillsLocations`. Esto es útil si quieres organizar skills en una estructura de carpetas diferente o tienes múltiples directorios de skills.

En un monorepo, habilita `chat.useCustomizationsInParentRepositories` para descubrir skills desde el repositorio padre raíz.

### Comando /create-skill

Usa `/create-skill` en el chat para generar un skill con IA. Describe el skill que deseas (por ejemplo, "a skill for running and debugging integration tests") y el agente hará preguntas de clarificación y generará un archivo `SKILL.md` con la estructura del directorio, instrucciones y frontmatter.

También puedes extraer un skill reutilizable de una conversación continua. Por ejemplo, después de una sesión multi-turn donde depuraste un problema complejo, pregunta "create a skill from how we just debugged that" para capturar el procedimiento multi-paso como un skill reutilizable.
...
|-------|-----------|-------------|
| `name` | Sí | Identificador único del skill. Debe ser minúsculas, usando guiones para espacios (ejemplo: `webapp-testing`). Máximo 64 caracteres. Debe coincidir con el nombre del directorio padre. |
...
| `argument-hint` | No | Hint de texto mostrado en el campo de input del chat cuando el skill se invoca como slash command. Ayuda a los usuarios a entender qué información adicional proporcionar (ejemplo: `[test file] [options]`). |
| `user-invocable` | No | Controla si el skill aparece como slash command en el menú de chat. Por defecto `true`. Establecer en `false` para ocultar el skill del menú `/` mientras se permite que el agente lo cargue automáticamente. |
| `disable-model-invocation` | No | Controla si el agente puede cargar automáticamente el skill basado en relevancia. Por defecto `false`. Establecer en `true` para requerir invocación manual solo a través del slash command `/`. |

### Tabla resumen de propiedades de frontmatter

| Propiedad | default | slash command | Caso de uso |
|-----------|---------|---------------|-------------|
| Default (ambas omitidas) | Yes | Yes | Skills de propósito general |
| `user-invocable: false` | No | Yes | Background knowledge skills que el modelo carga cuando es relevante |
| `disable-model-invocation: true` | Yes | No | Skills que solo quieres ejecutar bajo demanda |
| Ambas configuradas | No | No | Skills deshabilitados |
...
Agent Skills usa carga eficiente de contexto en tres niveles:
...
### Cómo Copilot usa los skills

Ejemplo con el skill `webapp-testing`:

1. **Discovery**: Copilot lee el `name` y `description` del skill desde el YAML frontmatter. Cuando preguntas "help me test the login page", Copilot hace match con el skill `webapp-testing` basándose en su descripción.
2. **Instructions loading**: Copilot carga el cuerpo del `SKILL.md` en su contexto, dándole acceso a los procedimientos detallados de testing y guidelines. También puedes activar este paso directamente escribiendo `/webapp-testing` en chat.
3. **Resource access**: Mientras Copilot trabaja con las instrucciones, accede a archivos adicionales en el directorio del skill (como `test-template.js` o escenarios de ejemplo) solo cuando los referencia. Si un archivo no está referenciado en las instrucciones, no se carga.
...
## 6. Skills desde Extensiones

Las extensiones pueden contribuir skills usando el punto de contribución `chatSkills` en su `package.json`. La ruta debe apuntar a un directorio que contiene un archivo `SKILL.md`, siguiendo la [Agent Skills specification](https://agentskills.io/specification).
...
extension-root/
└── skills/
    └── my-skill/           # El nombre del directorio debe coincidir con el campo `name` en SKILL.md
        └── SKILL.md         # Requerido
```

### Registrar el skill en package.json
...
> **Importante**: El campo `name` en el frontmatter del `SKILL.md` debe coincidir con el nombre del directorio padre. Por ejemplo, si el directorio es `skills/my-skill/`, el campo `name` debe ser `my-skill`. Si el nombre no coincide, el skill no se carga.

---

## 7. Skills Compartidos
