...
### Ubicaciones de Archivos de Custom Agents

Los custom agents pueden definirse a nivel de workspace o de perfil de usuario. La siguiente tabla muestra las ubicaciones predeterminadas según el alcance:

| Alcance | Ubicación |
|---------|-----------|
| Workspace | `.github/agents` (formato VS Code) o `.claude/agents` (formato Claude) |
| Usuario | `~/.copilot/agents` o en el perfil de usuario de VS Code |

Para workspaces monot repositorio, se puede habilitar `chat.useCustomizationsInParentRepositories` para descubrir custom agents desde el directorio raíz del repositorio padre. Se pueden configurar ubicaciones adicionales mediante el setting `chat.agentFilesLocations`.
...
## 1.5 Handoffs (Transiciones)

### Concepto de Handoffs

Los **handoffs** permiten crear flujos de trabajo secuenciales guiados que transicionan entre agentes con pasos sugeridos siguiente. Después de completar una respuesta del chat, aparecen botones de handoff que permiten al usuario moverse al siguiente agente con contexto relevante y un prompt pre-llenado. Esta característica es útil para orquestar flujos de trabajo multi-paso donde el desarrollador mantiene control para revisar y aprobar cada paso antes de continuar.

### Flujos de Trabajo con Handoffs

Los patrones de handoff más comunes incluyen:

- **Planificación → Implementación**: Generar un plan en el agente de planificación, luego transferir al agente de implementación para comenzar a codificar
- **Implementación → Revisión**: Completar la implementación, luego cambiar al agente de revisión de código para verificar calidad y seguridad
- **Tests Fallidos → Tests Pasando**: Generar tests fallidos que son más fáciles de revisar que implementaciones grandes, luego transferir para hacer pasar los tests implementando los cambios requeridos

### Configuración de Handoffs en el Frontmatter

Los handoffs se definen en el frontmatter del archivo `.agent.md` con la siguiente estructura:
...
Las propiedades de cada handoff son:

- **label**: Texto que aparece en el botón de transición
- **agent**: Identificador del agente destino
- **prompt**: Texto del prompt a enviar al agente destino
- **send**: (opcional) Si `true`, el prompt se envía automáticamente; si `false`, el usuario debe confirmar antes de enviar
- **model**: (opcional) Modelo de lenguaje a usar cuando se ejecuta el handoff
...
La propiedad **agents** especifica una lista de nombres de agentes disponibles como subagentes dentro de este agente. Se puede usar `*` para permitir todos los agentes, o un array vacío `[]` para prevenir cualquier uso de subagentes. Si se especifican agentes, asegúrate de incluir la herramienta `agent` en la propiedad `tools`. Para crear un agente auto-referencial que se liste a sí mismo en `agents`, habilita `chat.subagents.allowInvocationsFromSubagents`.

### Selección de Modelo y Configuración Avanzada

La propiedad **model** especifica el modelo de IA a utilizar cuando se ejecuta el prompt del agente. Se puede especificar un único nombre de modelo (string) o una lista priorizada de modelos (array). Cuando se especifica un array, el sistema prueba cada modelo en orden hasta encontrar uno disponible. Si no se especifica, se utiliza el modelo actualmente seleccionado en el selector de modelos del chat.

La propiedad **user-invocable** es un flag booleano opcional que controla si el agente aparece en el dropdown de agentes en el chat (el valor predeterminado es `true`). Establecer en `false` crea agentes que solo son accesibles como subagentes o programáticamente.

La propiedad **disable-model-invocation** es un flag booleano opcional para prevenir que el agente sea invocado como subagente por otros agentes (el valor predeterminado es `false`).

La propiedad **infer** está deprecada. En su lugar, usa `user-invocable` y `disable-model-invocation` para control independiente. Anteriormente, `infer: true` (valor predeterminado) hacía que el agente fuera visible tanto en el selector como disponible como subagente. `infer: false` lo ocultaba de ambos. Ahora puedes usar `user-invocable: false` para ocultar del selector mientras permites invocación como subagente, o `disable-model-invocation: true` para prevenir invocación como subagente mientras se mantiene visible en el picker.

La propiedad **target** define el entorno objetivo o contexto para el custom agent. Los valores posibles son `vscode` para el agente local de VS Code o `github-copilot` para agentes en GitHub Copilot.

La propiedad **mcp-servers** es una lista opcional de configuración JSON de servidores MCP (Model Context Protocol) para usar con custom agents en GitHub Copilot (target: github-copilot).

La propiedad **hooks** (Preview) permite definir comandos de hooks específicos para este agente. Los hooks definidos aquí solo se ejecutan cuando este agente está activo, ya sea invocado por el usuario o como subagente. Requiere que `chat.useCustomAgentHooks` esté habilitado.
...
## 6.5 Crear un Custom Agent con IA

VS Code permite generar custom agents usando AI. En el modo chat de Agent, escribe `/create-agent` y describe el rol deseado (por ejemplo, "a security review agent"). El agente hacerte preguntas clarificadoras y genera un archivo `.agent.md` con herramientas, instrucciones y frontmatter apropiados.

También puedes extraer un custom agent desde una conversación en curso. Por ejemplo, después de una sesión de debugging multi-turno, pregunta "make an agent for this kind of task" para capturar el workflow como un custom agent reutilizable.

Alternativamente, puedes generar un custom agent desde el Chat Customizations editor seleccionando "Generate Agent" desde el dropdown.

## 6.6 Formato Claude Agent

VS Code también soporta archivos de agentes en formato Claude en la carpeta `.claude/agents`. Estos archivos `.md` usan propiedades de frontmatter específicas de Claude:

| Propiedad | Descripción |
|-----------|-------------|
| name | Nombre del agente (requerido) |
| description | Qué hace el agente |
| tools | String separado por comas de herramientas permitidas (ej: "Read, Grep, Glob, Bash") |
| disallowedTools | String separado por comas de herramientas bloqueadas |

VS Code mapea los nombres de herramientas específicos de Claude a las herramientas correspondientes de VS Code. Ambos formatos (VS Code `.agent.md` con arrays YAML y Claude con strings separados por comas) son soportados.

VS Code también detecta archivos `.md` en la carpeta `.claude/agents`, siguiendo el formato de subagentes de Claude. Esto permite usar las mismas definiciones de agentes en VS Code y Claude Code.

## 6.7 Compartir Agents entre Equipos

Para compartir custom agents entre equipos, puedes crear un custom agent a nivel de workspace (carpeta `.github/agents`). Si quieres compartir agents entre múltiples workspaces dentro de tu organización, puedes definirlos a nivel de organización de GitHub.

VS Code detecta automáticamente custom agents definidos a nivel de organización a los que tu cuenta tiene acceso. Estos agents aparecen en el dropdown de Agents junto con los built-in agents, tus agents personales y los del workspace.

Para habilitar el descubrimiento de custom agents a nivel de organización, establece `github.copilot.chat.organizationCustomAgents.enabled` en `true`.

## 6.8 Security Considerations

Los custom agents pueden restringir qué herramientas están disponibles, lo que te da control sobre lo que la IA puede hacer. Para workflows sensibles a seguridad, crea agentes con herramientas de solo lectura para prevenir modificaciones no intencionadas. Cuando compartas agentes en un repositorio, revisa la lista de herramientas e instrucciones para asegurar que siguen el principio de menor privilegio.
